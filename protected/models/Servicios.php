...
class Servicios extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('parrafo, titulo', 'required', 'message' => '{attribute} no debe estar vacio.'),
			array('estado', 'numerical', 'integerOnly'=>true,'message' => '{attribute} solo debe ser numeros.'),
			array('image', 'length', 'max'=>255),
			array('button, link, created_at', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, image, parrafo, titulo, button, link, created_at, estado', 'safe', 'on'=>'search'),
		);
	}
