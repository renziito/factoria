...
class Slider extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('title', 'required', 'message' => '{attribute} no debe estar vacio.'),
			array('estado', 'numerical', 'integerOnly'=>true,'message' => '{attribute} solo debe ser numeros.'),
			array('image, button, position, link', 'length', 'max'=>255),
			array('created_at', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, image, title, button, position, link, created_at, estado', 'safe', 'on'=>'search'),
		);
	}
